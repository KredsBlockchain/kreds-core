...
###   This script attempts to download the signature file SHA256SUMS.asc from kreds.org
...
WORKINGDIR="/tmp/kreds_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://kreds.org/bin/"
VERSIONPREFIX="kreds-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kreds-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Kreds binary release signing key installed?"
   fi
