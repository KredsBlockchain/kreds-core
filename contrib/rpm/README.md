...
The RPM spec file provided here is for Kreds-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kreds.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kreds git repository](https://github.com/kreds/kreds) but are not part of
...
Currently only `Source10` is used, for the example `kreds.conf` file.
...
in the kreds git repository. At present only an SVG version of the Kreds
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kreds
...
needed when 0.12.1 is released, a proper fix is already in the Kreds git
master branch.
...
idea to build Kreds against a static version of that library compiled
...
The `kreds.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kreds-server` package. At this point, most RPM based
...
The `kreds-server` package should have SELinux support. How to properly do
...
Kreds-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kreds/). He has been
packaging Kreds for Fedora at least since 2012.
...
4. I renamed the `kreds` package that contains the Qt GUI to `kreds-core` as
that appears to be how the general population refers to it, in contrast to
`kreds-xt` or `kreds-classic`. I wanted to make sure the general population
