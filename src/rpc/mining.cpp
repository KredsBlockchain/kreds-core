...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated kreds to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/kreds/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/kreds/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/kreds/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/kreds/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Kreds is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Kreds is downloading blocks...");
...
UniValue submitblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.kreds.it/wiki/BIP_0022 for full specification.\n"
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                && strMode != "protocol" && strMode != "full" && strMode != "votes" && strMode != "donation" && strMode != "pose"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Kreds initiative (can be additionally filtered, partial match)\n"
                );
    }
...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in kreds.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  stop         - Stop masternode configured in kreds.conf\n"
    ...
    if (strCommand == "genkey") {
        ...
        return CKredsSecret(secret).ToString();
    }
...
/*    
    ...
    return ""; // Fix compile warning for no return value
}
